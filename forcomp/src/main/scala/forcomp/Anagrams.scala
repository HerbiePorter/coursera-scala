...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase
    .groupBy(identity)
    .map((group: (Char, String)) => (group._1, group._2.length))
    .toList
    .sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case head :: tail =>
      ...
      (for {
        elementVar <- 1 to head._2
        s <- subsets
      } yield (head._1, elementVar) :: s).toList ++ subsets
  }
  ...
  /** Subtracts occurrence list `y` from occurrence list `x`.
    ...
    */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = y.foldLeft(x.toMap)((acc, next) => {
    val newValue = acc(next._1) - next._2
    if (newValue > 0) {
      acc + (next._1 -> newValue)
    } else {
      acc - next._1
    }
  }).toList
    ...
    def anagrams(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case list =>
        for {
          ...
          words <- anagrams(subtract(occurrences, subset))
        } yield word :: words
    }
